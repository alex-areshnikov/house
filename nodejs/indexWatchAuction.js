...
const logger = new ApiLogger(`watch-auction-lot-${lotNumber}`);

try {
  await logger.say("Starting browser routine")

  const loginner = new Loginner(logger, username, password);
  const auctionWatcher = new AuctionWatcher(logger, lotNumber);

  await loginner.login();

  if(loginner.isSuccess()) { await auctionWatcher.watch(loginner.loggedInPage()) }
